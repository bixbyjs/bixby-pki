...
exports = module.exports = function(settings, logger) {
  ...
  if (!config.format) { config.format = 'pem'; }
  ...
  return function keygen(cb) {
    ...
    if (config.format == 'pem') {
      switch (config.algo) {
      case 'rsa':
        return require('./generate/pem-rsa')({ bits: config.bits }, generated);
      default:
        return cb(new Error('Unsupported key type: ' + config.algo));
      }
    } else {
      return cb(new Error('Unsupported key format: ' + config.format));
    }
  };
}
